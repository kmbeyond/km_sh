...
if [ -z "$VISION_HOME" ]; then
	...
	if [ -d "$VISION_HOME" ]; then
		if [ -f "$VISION_HOME/conf/vision.conf" ]; then
			...
			log "Raw data location in HDFS: $HDFSEventsBaseLoc"
...
iYear=$(date -d '7 months ago' '+%Y')
iMonth=$(date -d '7 months ago' '+%-m')

log "Deleting partition path= ${HDFSEventsBaseLoc}/year=${iYear}/month=${iMonth}"
hdfs dfs -rm -R -f "${HDFSEventsBaseLoc}/year=${iYear}/month=${iMonth}"

#Another month older as well
iYear=$(date -d '8 months ago' '+%Y')
iMonth=$(date -d '8 months ago' '+%-m')
